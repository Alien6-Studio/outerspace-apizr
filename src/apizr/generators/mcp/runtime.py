...
LOGGER = logging.getLogger("apizr.mcp")
...
def create_server(root: Path, plan: RuntimePlan) -> Server[dict[str, object]]:
    module = load_source(root, plan)
    bindings = {t["tool_name"]: (t, verify_binding(module, t)) for t in plan["tools"]}
    tools = [
        Tool(
            name=t["tool_name"],
    ...
    async def call_tool(
        ...
        try:
            ...
            LOGGER.exception(
                "Capability invocation failed: %s", contract["capability_id"]
            )
...
def main() -> None:
    ...
    plan: RuntimePlan = json.loads((root / "apizr-mcp.json").read_bytes())
    server = create_server(root, plan)
