"""Frozen v1 import allowance: common CPython 3.11–3.14 stdlib roots.

This is policy data, not a runtime availability check. Keep it independent of
sys.stdlib_module_names so reports do not change with the host interpreter.
Changes to this set require readiness policy-version review.
"""

STDLIB_ROOTS = frozenset(
    """
    __future__ _abc _aix_support _ast _asyncio
    _bisect _blake2 _bz2 _codecs _codecs_cn
    ...
    _ctypes _curses _curses_panel _datetime _dbm
    _decimal _elementtree _frozen_importlib _frozen_importlib_external _functools
    _gdbm _hashlib _heapq _imp _io
    _json _locale _lsprof _lzma _markupbase
    _md5 _multibytecodec _multiprocessing _opcode _operator
    _osx_support _overlapped _pickle _posixshmem _posixsubprocess
    _py_abc _pydecimal _pyio _queue _random
    _scproxy _sha1 _sha3 _signal _sitebuiltins
    _socket _sqlite3 _sre _ssl _stat
    _statistics _string _strptime _struct _symtable
    _thread _threading_local _tkinter _tokenize _tracemalloc
    _typing _uuid _warnings _weakref _weakrefset
    _winapi _zoneinfo abc antigravity argparse
    array ast asyncio atexit base64
    bdb binascii bisect builtins bz2
    cProfile calendar cmath cmd code
    codecs codeop collections colorsys compileall
    concurrent configparser contextlib contextvars copy
    copyreg csv ctypes curses dataclasses
    datetime dbm decimal difflib dis
    doctest email encodings ensurepip enum
    errno faulthandler fcntl filecmp fileinput
    fnmatch fractions ftplib functools gc
    genericpath getopt getpass gettext glob
    graphlib grp gzip hashlib heapq
    hmac html http idlelib imaplib
    ...
    tarfile tempfile termios textwrap this
    ...
    winsound wsgiref xml xmlrpc zipapp
